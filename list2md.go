...
func main() {
	...
	for _, url := range lines {
		if strings.HasPrefix(url, "https://github.com/") {
			...
			issueAPI := fmt.Sprintf("https://api.github.com/repos/%s/issues?access_token=%s&state=all&since=%s", strings.TrimFunc(url[19:], trimSpaceAndSlash), accessToken, time.Now().AddDate(0, -1, 0).Format("2006-01-02T15:04:05Z"))
